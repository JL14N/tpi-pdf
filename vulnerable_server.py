...
app.secret_key = 'vulnerable-demo-secret'
...
def create_thumbnail(pdf_path, thumb_path, title=None):
    """Create a simple PNG thumbnail for the PDF's first page.
    This draws the document title or first lines of text onto an image as a lightweight preview.
    """
    try:
...
def index():
    ...
    return render_template('index.html', filename=last, admin_email=state.get('admin_email'), role=role, user=user, slots=slots, thumbs_available=thumbs_available, theme='vulnerable')
...
def login():
    ...
    # Render the login page for GET or after a failed POST
    return render_template('login.html', theme='vulnerable')
...
@require_login
def manage():
    # only attacker can manage uploads
    ...
    try:
        ...
        pass
    return render_template('manage.html', slots=slots, thumbs_available=thumbs_available, theme='vulnerable')
...
def sample_csrf_link():
    # Generate a PDF containing a visible link that performs a GET to change admin email
    buf = BytesIO()
    p = canvas.Canvas(buf)
    p.drawString(100, 750, 'Documento de prueba - Enlace CSRF')
    p.drawString(100, 730, 'Haga clic en el enlace para visitar la URL objetivo que cambia el email administrativo (GET).')
...
def sample_risky():
    ...
    # For this demo we redirect to the CSRF-link sample which exists in this app.
...
def admin_change_email():
    ...
    if request.method == 'GET':
        # If an email query param is present, update (admin only).
        ...
        if email:
            ...
            save_state(state)
            return f"Admin email changed from {old} to {email}", 200
        # Otherwise render a simple change form for admin
        state = load_state()
        return render_template('change_email.html', current=state.get('admin_email'), theme='vulnerable')
